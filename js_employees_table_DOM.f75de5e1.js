...
function setupTableSorting() {
    ...
    headers.forEach((header, index)=>{
        ...
        header.addEventListener('click', ()=>{
            ...
            if (currentSort.column === columnIndex) currentSort.direction = currentSort.direction === 'asc' ? 'desc' : 'asc';
...
function handleFormSubmit(e) {
    ...
    if (nameF.length < 4) {
        showNotification('Name too short', 'Name must be at least 4 characters long', 'error');
        return;
    }
    if (position.length < 4) {
        showNotification('Position too short', 'Position must be at least 4 characters long', 'error');
        return;
    }
    if (age < 18) {
        showNotification('Age too young', 'Employee must be at least 18 years old', 'error');
        return;
    }
    if (age > 90) {
        showNotification('Age too old', 'Employee age cannot exceed 90 years', 'error');
        return;
    }
    addEmployee({
        name: nameF,
